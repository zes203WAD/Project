...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function save(){
    ...
    if(itemsDiv){
      itemsDiv.innerHTML = cart.length ? cart.map(i=>`<div class="flex gap-3 mb-3 border-b pb-3"><img src="${i.image}" alt="${i.name}" class="w-12 h-12 rounded object-cover"><div><p class="text-sm font-bold">${i.name}</p><p class="text-xs">$${i.price} x ${i.qty}</p>
      </div>
